...
# to act as the SOURCE for mongosync.
# mongosync requires at least mongo 6.0 (both src and dest).
# mongosync 1.1 requires at least mongo 6.0.4 (both src and dest).
# mongod must be in your path for the launch command to succeed.
...
do 
  ...
  db_path=$BASE_DB_PATH/src/db$db_port/
  mkdir -p $db_path
...
sed -i '' "s|^MDB_URI_SRC.*|MDB_URI_SRC=$MDB_URI|g" demo.conf

# Writes the Node Count to demo.conf
sed -i '' "s|^NODE_COUNT_SRC.*|NODE_COUNT_SRC=$node_count|g" demo.conf

echo
echo "demo.conf updated."
echo "MDB_URI_SRC set to $MDB_URI"
echo "NODE_COUNT_SRC set to $node_count"
echo
